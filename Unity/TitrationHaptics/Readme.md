...
3. 
**Transport**: The payload is sent via UDP over WiFi to the headset at approximately 60Hz to minimize latency.


4. 
**Reception**: A background thread in Unity listens for UDP packets, which are then parsed by `JsonUtility` to update the virtual state.
